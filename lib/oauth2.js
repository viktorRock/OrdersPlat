...
const extraGoogleScope = config.get('GOOGLE_SPREADSHEET_SCOPE');
...
function extractProfile (profile) {
  ...
  return {
    ...
    image: imageUrl
    ,tolkien: profile.tolkien
  };
}

function extractTolkiens (accessToken, refreshToken) {
  return {
    acess: accessToken,
...
passport.use(new GoogleStrategy({
  ...
  profile.tolkien = accessToken;
  // Extract the minimal profile information we need from the profile object
  // provided by Google
  cb(null, extractProfile(profile), extractTolkiens(accessToken, refreshToken));
}));
...
router.get(
  ...
  passport.authenticate('google', { scope: ['email', 'profile',extraGoogleScope] })
  );
// [END authorize]
...
router.get(
  ...
  (req, res) => {
    ...
    // console.log(req.authInfo);
    console.log(req.session.oauth2retur);
